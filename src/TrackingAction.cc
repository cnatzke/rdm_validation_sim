...
void TrackingAction::PostUserTrackingAction(const G4Track* track)
{
    ...
    if (nbtrk) {
        ...
        for (size_t itr=0; itr<nbtrk; itr++) {
            ...
            //exclude gamma de-excitation from momentum balance
            if (particle != G4Gamma::Gamma()) Pbalance += trk->GetMomentum();

            G4int pid               = trk->GetDynamicParticle()->GetPDGcode();
            G4double ekin           = trk->GetKineticEnergy();
            G4ThreeVector vertex    = trk->GetPosition();
            G4ThreeVector direction = trk->GetMomentumDirection();

            //if(trk->GetCreatorProcess() != nullptr) {
            //    G4String process_name = trk->GetCreatorProcess()->GetProcessName();
            //    G4cout << event_number << " | "
            //           << pid << " | "
            //           << ekin << " | "
            //           << process_name << " | "
            ...
            analysis->FillNtupleIColumn(1,pid);
            analysis->FillNtupleDColumn(2,ekin);
            analysis->FillNtupleDColumn(3,x);
            analysis->FillNtupleDColumn(4,y);
            analysis->FillNtupleDColumn(5,z);
            analysis->FillNtupleDColumn(6,theta);
            analysis->FillNtupleDColumn(7,phi);
